...
class PV2Server {
    ...
    setupServer() {
        this.server.on('connection', (socket) => {
            console.log(`PV2 Modbus client connected to port ${this.port}`);
            ...
            socket.on('end', () => {
                console.log(`PV2 Modbus client disconnected from port ${this.port}`);
                this.connections.delete(socket);
            });
            
            socket.on('error', (err) => {
                console.error(`PV2 Modbus socket error on port ${this.port}:`, err);
                this.connections.delete(socket);
            });
        ...
        this.server.on('error', (err) => {
            console.error(`PV2 Modbus server error on port ${this.port}:`, err);
        });
        
        console.log(`PV2 Modbus server configured on port ${this.port}`);
    }

    handleModbusRequest(socket, data) {
        try {
            if (data.length < 8) {
                console.log(`PV2: Invalid Modbus request length: ${data.length}`);
                return;
            }
            ...
            console.log(`PV2: Modbus request - TID:${transactionId}, FC:${functionCode}, Unit:${unitId}, Len:${length}`);

            // Check protocol ID (should be 0 for Modbus)
            if (protocolId !== 0) {
                console.log(`PV2: Invalid protocol ID: ${protocolId}`);
            ...
            if (data.length !== length + 6) {
                console.log(`PV2: Length mismatch - expected: ${length + 6}, actual: ${data.length}`);
                const errorResponse = this.createErrorResponse(transactionId, unitId, functionCode, 0x0B);
                socket.write(errorResponse);
                return;
            }
            ...
            try {
                ...
                console.error(`PV2: Error processing function ${functionCode}:`, error);
                response = this.createErrorResponse(transactionId, unitId, functionCode, 0x04);
            }

            if (response) {
                socket.write(response);
                console.log(`PV2: Sent response - TID:${transactionId}, Len:${response.length}`);
            }
        } catch (error) {
            console.error(`PV2: Error handling Modbus request:`, error);
            try {
                if (data.length >= 8) {
                    const transactionId = data.readUInt16BE(0);
                    const unitId = data.readUInt8(6);
                ...
                console.error(`PV2: Failed to send error response:`, e);
            }
        }
    }
    ...
    readRegisters(transactionId, unitId, startAddress, quantity) {
        try {
            ...
            console.error(`PV2: Error reading registers:`, error);
    ...
    writeRegister(transactionId, unitId, address, value) {
        try {
            ...
            console.error(`PV2: Error writing register:`, error);
    ...
    writeRegisters(transactionId, unitId, startAddress, values) {
        try {
            ...
            console.error(`PV2: Error writing registers:`, error);
    ...
    updateRegisterData() {
        try {
            ...
            if (pvData.mpptVoltages) {
                for (let i = 0; i < 24; i++) {
                    const voltage = Math.round((pvData.mpptVoltages[i] || 0) * 10); // 0.1V units
            ...
            if (pvData.mpptCurrents) {
                for (let i = 0; i < 24; i++) {
                    const current = Math.round((pvData.mpptCurrents[i] || 0) * 10); // 0.1A units
                    this.registerData[32017 + (i * 2)] = current;
                }
            }
            
            // Update main system registers
            this.registerData[32064] = Math.floor(pvData.inputPower / 65536); // High word
            this.registerData[32065] = pvData.inputPower & 0xFFFF; // Low word
            this.registerData[32066] = Math.round(pvData.inputVoltage * 10); // 0.1V units
            this.registerData[32067] = Math.round(pvData.inputCurrent * 10); // 0.1A units
            this.registerData[32068] = Math.floor(pvData.outputPower / 65536); // High word
            this.registerData[32069] = pvData.outputPower & 0xFFFF; // Low word
            ...
            console.error('Error updating PV2 register data:', error);
        }
    }
    ...
    async start() {
        return new Promise((resolve, reject) => {
            this.server.listen(this.port, '0.0.0.0', () => {
                this.isRunning = true;
                console.log(`✅ PV2 Modbus server started on port ${this.port}`);
                resolve();
            });
            
            this.server.on('error', (err) => {
                console.error(`❌ Failed to start PV2 Modbus server on port ${this.port}:`, err);
                reject(err);
            });
        });
    }
    ...
    async stop() {
        return new Promise((resolve) => {
            if (this.server) {
                ...
                this.server.close(() => {
                    this.isRunning = false;
                    console.log(`✅ PV2 Modbus server stopped on port ${this.port}`);
                    resolve();
                });
            } else {
                resolve();
